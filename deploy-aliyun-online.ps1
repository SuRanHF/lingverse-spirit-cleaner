...
function Get-FlagText($Flags) {
    ...
    $items = @()
    if ($Flags.running) { $items += "cleaning" }
    if ($Flags.monitoringSpirit) { $items += "monitoring" }
    if ($Flags.autoTrialRunning) { $items += "trial" }
    if ($Flags.autoTreasureRunning) { $items += "treasure" }
    if ($Flags.autoInscriptionRunning) { $items += "inscription" }
    if ($items.Count -eq 0) { return "idle" }
    return ($items -join " / ")
}
...
function Write-HtmlResponse($Stream, $Stats) {
    ...
    foreach ($client in $Stats.clients) {
        $rows += "<tr><td><code>$(Escape-Html $client.id)</code></td><td>$(Escape-Html $client.version)</td><td>$(Escape-Html $client.page)</td><td>$(Escape-Html (Get-FlagText $client.flags))</td><td>$(Escape-Html $client.ip)</td><td>$($client.secondsAgo)s</td></tr>"
    }
    if ([string]::IsNullOrWhiteSpace($rows)) {
        $rows = '<tr><td colspan="6" class="muted">No online clients</td></tr>'
    }
...
<head>
  ...
  <title>LingVerse Online Stats</title>
  <style>
...
<body>
  <main>
    <h1>LingVerse Online Stats</h1>
    <div class="muted">Clients with heartbeat in the last $($Stats.windowSeconds) seconds are online. This page refreshes every 10 seconds.</div>
    <section class="summary">
      <div class="stat"><div class="num">$($Stats.online)</div><div>Online</div></div>
      <div class="stat"><div>$(Escape-Html $Stats.updatedAt)</div><div class="muted">Updated</div></div>
    </section>
    <table>
      <thead><tr><th>Client</th><th>Version</th><th>Page</th><th>Status</th><th>IP</th><th>Last heartbeat</th></tr></thead>
      <tbody>$rows</tbody>
    </table>
  </main>
</body>
